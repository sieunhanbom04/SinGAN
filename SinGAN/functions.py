...
def load_trained_pyramid(opt, mode_='train'):
    ...
    if(os.path.exists(dir)):
        Gs = torch.load('%s/Gs.pth' % dir, map_location=lambda storage, loc: storage)
        Zs = torch.load('%s/Zs.pth' % dir, map_location=lambda storage, loc: storage)
        reals = torch.load('%s/reals.pth' % dir, map_location=lambda storage, loc: storage)
        NoiseAmp = torch.load('%s/NoiseAmp.pth' % dir, map_location=lambda storage, loc: storage)
    else:
